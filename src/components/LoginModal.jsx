...
export default function LoginModal({ profile, isGuestLocked, onUpdateProfile, onClose }) {
  ...
  // Initialize Google Sign-in button when Google SDK is loaded and container is rendered
  useEffect(() => {
    if (OAUTH_CONFIG.USE_SANDBOX_DEV_MODE) return;
    
    // Check if modal is open (we can infer it since this component is rendered only when open)
    let intervalId;
    const checkAndRenderGoogle = () => {
      if (window.google && OAUTH_CONFIG.GOOGLE_CLIENT_ID !== 'YOUR_GOOGLE_CLIENT_ID.apps.googleusercontent.com') {
